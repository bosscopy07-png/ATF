...
async function showAccount(userId: number, chatId: number): Promise<void> {
  ...
  const caption = [
    ...
    totalNgn ? `≈ ₦${totalNgn}` : '',
    '',
    aftPrice ? `📈 AFT Price: <code>$${aftPrice.price.toFixed(6)}</code>` : '⚠️ Price temporarily unavailable',
    '',
...
async function showExportWarning(userId: number, chatId: number): Promise<void> {
  const caption = [
    ...
    '',
    '<b>AFTSwap support will NEVER ask for it.</b>',
  ].join('\n');
...
async function showHelp(userId: number, chatId: number): Promise<void> {
  const caption = [
    'ℹ️ <b>HELP</b>',
    '',
    '<b>AFTSwap</b> is a custodial TON ↔ AFT exchange.',
    '',
    '<b>Commands:</b>',
...
async function showUserDetail(userId: number, chatId: number, targetId: number): Promise<void> {
  try {
    ...
    const caption = [
      ...
      `🪙 AFT: ${Precision.formatDisplay(aftBalance)}`,
      '',
